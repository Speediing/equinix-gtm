...
fn agentComputer(p: vec2f) -> f32 {
  let q = rotate(p, -0.14);
  let bezel = sdRoundedBox(q, vec2f(0.21, 0.15), 0.025);
  let screen = sdRoundedBox(q - vec2f(0.0, -0.01), vec2f(0.175, 0.11), 0.012);
  let stem = sdRoundedBox(q - vec2f(0.0, 0.18), vec2f(0.035, 0.06), 0.01);
  let base = sdRoundedBox(q - vec2f(0.0, 0.245), vec2f(0.11, 0.018), 0.008);
  var mark = opSmoothUnion(bezel, stem, 0.014);
  mark = opSmoothUnion(mark, base, 0.012);
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let equinixRed = vec3f(0.929412, 0.109804, 0.141176);
  ...
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(equinixRed, paper, spark * 0.45 + line * 0.05);
